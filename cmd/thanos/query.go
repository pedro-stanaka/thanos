...
func registerQuery(app *extkingpin.App) {
	...
	extendedFunctionsEnabled := cmd.Flag("query.enable-x-functions", "Whether to enable extended rate functions (xrate, xincrease and xdelta). Only has effect when used with Thanos engine.").Default("true").Bool()
...
func runQuery(
	...
	if queryMode != queryModeLocal {
		remoteEngineEndpoints = query.NewRemoteEndpoints(logger, reg, endpoints.GetQueryAPIClients, query.Opts{
	...
	// Start query API + UI HTTP server.
	{
		...
		ui.NewQueryUI(logger, endpoints, webExternalPrefix, webPrefixHeaderName, alertQueryURL, receive.DefaultTenantHeader, receive.DefaultTenant, false).Register(router, ins)
	...
	// Start query (proxy) gRPC StoreAPI.
	{
		...
		grpcAPI := apiv1.NewGRPCAPI(time.Now, queryReplicaLabels, queryableCreator, engineFactory, defaultEngineType, lookbackDeltaCreator, instantDefaultMaxSourceResolution, log.With(logger, "sub_component", "query_grpc"))
