		...
		t.Run(testName, func(t *testing.T) {
			t.Run("proto_writer", func(t *testing.T) {
				...
				for idx, req := range testData.reqs {
					err := w.Write(context.Background(), DefaultTenant, req.Timeseries)
			...
			t.Run("capnproto_writer", func(t *testing.T) {
				...
				for idx, req := range testData.reqs {
					capnpReq, err := writecapnp.Build(DefaultTenant, req.Timeseries)
					testutil.Ok(t, err)

					wr, err := writecapnp.NewRequest(capnpReq)
					testutil.Ok(t, err)
					err = w.Write(context.Background(), DefaultTenant, wr)
